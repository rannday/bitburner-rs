...
use tungstenite::{HandshakeError, WebSocket, accept};
...
impl NativeWebSocketTransport {
    pub fn from_stream(stream: TcpStream) -> Result<Self> {
        stream
            ...
            .map_err(|err| BitburnerError::io("set websocket read timeout", err))?;
        stream
            .set_write_timeout(Some(DEFAULT_REQUEST_TIMEOUT))
            .map_err(|err| BitburnerError::io("set websocket write timeout", err))?;
        let socket = accept(stream).map_err(|err| match err {
            HandshakeError::Failure(err) => BitburnerError::websocket(
    ...
    pub fn close(&mut self) -> Result<()> {
        self.socket
            .close(None)
            .map_err(|err| BitburnerError::websocket("close websocket", err))
    }

    fn read_response_text(&mut self, method: &str) -> Result<String> {
        loop {
            let message = self.socket.read().map_err(|err| {
                BitburnerError::websocket(
                    format!("{method} timed out or failed waiting for response"),
                    err,
                )
            })?;
            match message {
                ...
                Message::Ping(bytes) => {
                    self.socket.send(Message::Pong(bytes)).map_err(|err| {
                        BitburnerError::websocket(
                            format!("send pong while waiting for {method}"),
                            err,
                        )
                    })?;
                }
...
impl BitburnerTransport for NativeWebSocketTransport {
    fn send_request_value(&mut self, request: JsonRpcRequest) -> Result<JsonRpcResponse<Value>> {
        ...
        let text = self.read_response_text(&method)?;
